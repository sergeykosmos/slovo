...
class ButtonGrid(GridLayout):
    def __init__(self, **kwargs):
        global color_but, color_tabl, buttons, vvod_slov, massiv_slov_our, slovo_na_ekrane
        ...
        button_image = Image(source='C:/Users/Admin/Desktop/Проэекты программ/slova_tinkoff/info.jpg', size=(30, 30))
        icon_button = Button(background_normal='', size_hint=(None, None), size=(30, 30))
        icon_button.bind(on_release=self.info_but)
        icon_button.add_widget(button_image)
        label_belii.add_widget(icon_button)

        middle_button_1 = Button(text='start', size_hint=(None, None), size=(80, 50),
                                 pos=(belii_width * 1, belii_height / 2))  #\
        middle_button_1.bind(on_release=lambda instance: start())
        label_belii.add_widget(middle_button_1)
        
        middle_button_2 = Button(text='next', size_hint=(None, None), size=(80, 50),
                                 pos=(self.width * 3, belii_height / 2))
   ...
   # def change_color(self, instance):
    ...
    def info_but(self, instance):
        text_info = '''
        1. Введите слово, 
        заменяя звездочками буквы.
        2. Возможны следующие варианты:
        2.1 пишем слово, 
        заменяя звездочками 
        все неизвестные буквы. 
        Причем буквы стоят на 
        нужных позициях.
        2.2 пишем буквы, 
        какие есть в слове: is: крб
        2.3 пишем буквы, 
        каких нет в слове: no: крб
        3. На каждом этапе
        будут выводиться 
        первые 10 слов наиболее 
        подходящих под критерии, 
        рядом с ними будут написаны веса. 
        Чем больше вес, 
        тем более подходящее слово.
        Введите слово, 
        заменяя буквы звездочками:
        '''

        layout = GridLayout(cols=1)
        scroll_view = ScrollView(size_hint=(None, None), size=(400, 400), pos=(0, 200))
        label = Label(text=text_info, size_hint=(0.8, None), height=self.height * 0.8)  #
        scroll_view.add_widget(label)
        layout.add_widget(scroll_view)

        popup = Popup(title='Инструкция', content=layout, size_hint=(None, None), size=(400, 500))
        popup.open()
...
def start():
    ...
    for i1 in range(5):
        for i2 in range(5):
            ...
            if i1 == 0:
                buttons[(i1, i2)].text = (massiv_slov_our[0])[i2]
            else:
                buttons[(i1, i2)].text =""
...
def next():
 ...
 #   print(len(massiv_slov_our_pred))
    ...
    if len(massiv_slov_our)>=10:
        slovo_na_ekrane = massiv_slov_our_pred[0]
...
adres = 'C:/Users/Admin/Desktop/Project/slova_tinkoff/2.0/singular.txt'#russian.txt

#adres = resource_filename('singular', 'singular.txt')
